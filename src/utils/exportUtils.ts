 ...
 */
export function drawTileToCanvas(
  ...
  const scale = targetWidth / settings.artboardSize;
  ...
  if (!settings.backgroundTransparent) {
    ...
    ctx.fillRect(0, 0, settings.artboardSize, settings.artboardSize);
  } else {
    ctx.clearRect(0, 0, settings.artboardSize, settings.artboardSize);
  }

  // Clip to 1:1 artboard boundary if requested
  if (clipToArtboard) {
    ctx.beginPath();
    ctx.rect(0, 0, settings.artboardSize, settings.artboardSize);
    ctx.clip();
  }
  ...
  for (const el of sorted) {
    // Render all toroidal wrapped instances (center and wraps)
    const instances = getToroidalInstances(el, settings.artboardSize);
    ...
    for (const inst of instances) {
      ...
      if (el.type === 'shape' && el.shapeKind) {
        ...
        const path2D = new Path2D(preset.path);

        // Normalize 100x100 SVG path to element width/height centered at 0,0
        ...
        const path2D = new Path2D(el.pathData);
        ctx.translate(-el.width / 2, -el.height / 2);
        if (el.fill && el.fill !== 'none') {
          ctx.fillStyle = el.fill;
          ctx.fill(path2D);
        ...
        ctx.font = `${el.fontSize || 32}px ${el.fontFamily || 'Plus Jakarta Sans'}, sans-serif`;
 ...
 */
export function exportHighResTile(
  elements: DesignElement[],
  settings: PatternSettings,
  ...
  drawTileToCanvas(ctx, elements, settings, size, size, true);
 ...
 */
export function exportTiledFabric(
  elements: DesignElement[],
  settings: PatternSettings,
  repeatCols: number = 4,
  repeatRows: number = 4,
  ...
  if (!tileCtx) return;
  drawTileToCanvas(tileCtx, elements, settings, tileSizePx, tileSizePx, true);
 ...
 */
export function exportFabricSpecSheet(
  elements: DesignElement[],
  settings: PatternSettings
): void {
  ...
  const tileCtx = tileCanvas.getContext('2d');
  if (tileCtx) {
    drawTileToCanvas(tileCtx, elements, settings, tileW, tileH, true);
