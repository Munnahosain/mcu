...
export const MockupView: React.FC<MockupViewProps> = ({
  ...
  const [tileDataUrl, setTileDataUrl] = useState<string>('');

  // Generate 1:1 seamless tile data URL whenever elements or settings change
  useEffect(() => {
    ...
    setTileDataUrl(canvas.toDataURL('image/png'));
  }, [elements, settings]);

  const tileSize = Math.round(120 * patternScale);

  const productTabs: { id: ProductMockup; name: string; icon: any }[] = [
