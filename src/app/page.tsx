...
const tools = [
  { icon: WandSparkles, label: "AI metadata", title: "Turn images into publishable listings", text: "Generate titles, descriptions, categories, keywords, and prompts with provider-aware controls and batch processing.", href: "/dashboard/generator", color: "bg-primary/10" },
  { icon: Box, label: "3D icon studio", title: "Build the hero asset", text: "Shape SVGs into Liquid Glass, Plastic, or Metal icons, tune bevels and lighting, then export PNG, GLB, OBJ, or MP4.", href: "/dashboard/3d-icon-studio", color: "bg-cyan-500/10" },
  { icon: Eraser, label: "Background remover", title: "Deliver transparent assets", text: "Remove image backgrounds in a bulk queue and download clean PNG results for your next listing or composition.", href: "/dashboard/bg-remover", color: "bg-emerald-500/10" },
  { icon: Grid3X3, label: "Grid + Bento", title: "Compose the visual system", text: "Create Fibonacci, Golden Ratio, isometric, and bento layouts with presets, widgets, and SVG export.", href: "/dashboard/grid-generator", color: "bg-amber-500/10" },
  { icon: Palette, label: "Palette studio", title: "Extract a usable color language", text: "Pull colors from an image, refine the palette, and copy CSS variables or download JSON and SVG swatches.", href: "/dashboard/palette", color: "bg-rose-500/10" },
  { icon: BarChart3, label: "Trading studio", title: "Make data-ready visuals", text: "Build candlestick charts with indicators, templates, favorites, realtime market data, and SVG or PNG export.", href: "/dashboard/trading", color: "bg-violet-500/10" },
];
      ...
      <section id="features" className="border-y border-foreground/[0.07] bg-foreground/[0.018]">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {tools.map(({ icon: Icon, label, title, text, href, color }) => <Link key={title} href={href} className="group rounded-3xl border border-foreground/[0.09] bg-background/55 p-6 transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${color} text-primary`}><Icon className="h-5 w-5" /></div><p className="mt-5 text-xs font-semibold text-primary">{label}</p><h3 className="mt-2 text-lg font-extrabold">{title}</h3><p className="mt-2 text-sm leading-relaxed text-foreground/62">{text}</p><span className="mt-6 inline-flex items-center gap-1 text-xs font-bold text-primary">Open tool <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1" /></span></Link>)}
          </div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start"><div><p className="text-sm font-semibold text-primary">Also in the workspace</p><h2 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Small tools that finish the job.</h2><p className="mt-4 text-sm leading-relaxed text-foreground/65">When the main asset is ready, these focused surfaces help you package, plan, and polish it.</p></div><div className="grid gap-3 sm:grid-cols-2">{supportingTools.map(([Icon, title, text]) => <div key={title as string} className="flex gap-3 rounded-2xl border border-foreground/[0.08] p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-4 w-4" /></span><div><h3 className="text-sm font-extrabold">{title as string}</h3><p className="mt-1 text-xs leading-relaxed text-foreground/60">{text as string}</p></div></div>)}</div></div>
      </section>
